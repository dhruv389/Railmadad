...
const SendComplaintToStaff = async (req, res) => {
   ...
   console.log(complaintId);
    ...
    try {
      ...
      res.status(500).json({ message: 'Error sending complaint to staff', error: err.message });
    }
}
...
const AfterResolvedSendToAdmin = async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: 'Error sending complaint to Admin', error: err.message });
    }
}
...
module.exports = {getAdminCompaints,DeleteComplaint , SendComplaintToStaff , AfterResolvedSendToAdmin};